...
class FunctionNode(Node):
    def __init__(self, function, arity, parameters=None):
    ...
    def evaluate(self):
        ...
        return self.function(*results, *self.parameters)

    def __str__(self):
        params_str = ""
        if self.parameters:
            params_str = ", " + ", ".join(map(str, self.parameters))
        return f"F.{self.function.__name__}({', '.join(map(str, self.children))}{params_str})"
...
Func_set = [(F.Img_Sum, 2), (F.Img_Sub, 2), (F.Img_Multi, 2), (F.Img_Div, 2), (F.DX,1), (F.DY,1), (F.Filter_Gaussian1,1), (F.Log_Image,1), (F.Exp_Image,1), (F.Half_Image,1), (F.Sqrt_Image,1)]
Term_set = ["I", "R", "G", "B"]
...
def generate_random_tree(max_depth):
    if max_depth == 0:
        ...
        parameters = []
        if function == F.Filter_Gaussian:
            parameters = [random.choice(range(1, 52, 2))]
        node = FunctionNode(function, arity, parameters)
...
def select_distinct_parents(population, fitness_values):
    ...
    # Asegurarse de que parent1 y parent2 no sean el mismo individuo
...
def crossover(parent1, parent2):
    ...
    # Seleccionar puntos de cruce válidos
    crossover_point1 = random.choice([n for n in nodes1 if isinstance(n, FunctionNode)])
    crossover_point2 = random.choice([n for n in nodes2 if isinstance(n, FunctionNode)])
    
    # Intercambiar todo el nodo (función, parámetros e hijos)
    ...
    crossover_point1.parameters, crossover_point2.parameters = crossover_point2.parameters, crossover_point1.parameters
    ...
    def mutate_node(node):
        if random.random() < mutation_rate:
            if isinstance(node, FunctionNode):
                ...
                if node.function == F.Filter_Gaussian and node.parameters:
                    node.parameters[0] = random.choice(range(1, 52, 2)) 
                
                # Cambiar función (incluyendo nuevos parámetros)
                if random.random() < 0.5:
                    new_func, new_arity = random.choice(Func_set)
                    
                    # Generar nuevos parámetros si es necesario
                    new_params = []
                    if new_func == F.Filter_Gaussian:
                        new_params = [random.choice(range(1, 52, 2))]
...
def genetic_algorithm(population_size, generations, max_depth=5, crossover_rate=0.9, mutation_rate=0.1):
    ...
    global_max_fitness = -1  # Inicializar con un valor bajo
    global_best_individual = None  # Inicializar sin individuo
...
# EN: Run GA
best_tree = genetic_algorithm(population_size=5, generations=10, max_depth=5, crossover_rate=0.7, mutation_rate=0.3)
print("\nFinal Best Tree:")
